...
def get_objects_from_obs(obs, goal_override=None):
    all_objs = set()
    discs = set()
    disc_pair_order = set()
    ...
    on_links = {}
    for lit in (goal_override if goal_override is not None else obs):
...
def render(obs, mode='human', close=False, goal_override=None):
    if mode == 'layout' or mode == 'layout_goal':
        if mode == 'layout':
            assert goal_override is None
            pegs, discs_ordered_by_size, peg_to_disc_list = get_objects_from_obs(obs)
        else:
            assert goal_override is not None
            pegs, discs_ordered_by_size, peg_to_disc_list = get_objects_from_obs(obs, goal_override=goal_override)

        layout = [-1 for _ in range(len(discs_ordered_by_size))]
        for i, disc in enumerate(discs_ordered_by_size):
            for j, (peg, discs) in enumerate(peg_to_disc_list.items()):
                if disc in discs:
                    layout[i] = j
                    break
