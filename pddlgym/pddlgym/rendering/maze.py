...
NUM_OBJECTS = 10
WALL, PERSON, GOAL_UP_PLAYER, NOT_GOAL_UP_PLAYER, GOAL_LEFT_PLAYER, GOAL_DOWN_PLAYER, GOAL_RIGHT_PLAYER, NOT_GOAL_LEFT_PLAYER, NOT_GOAL_DOWN_PLAYER, NOT_GOAL_RIGHT_PLAYER = range(NUM_OBJECTS)
...
TOKEN_IMAGES = {
    ...
    # The above 4 is used in discrete representation
...
def layout_from_string_grid(grid):
    layout = np.zeros((grid.shape[0], grid.shape[1]), dtype=int)
    layout[grid == 'goal_up_player'] = GOAL_UP_PLAYER
    layout[grid == 'goal_left_player'] = GOAL_UP_PLAYER
    layout[grid == 'goal_down_player'] = GOAL_UP_PLAYER
    layout[grid == 'goal_right_player'] = GOAL_UP_PLAYER
    layout[grid == 'not_goal_up_player'] = NOT_GOAL_UP_PLAYER
    layout[grid == 'not_goal_left_player'] = NOT_GOAL_UP_PLAYER
    layout[grid == 'not_goal_down_player'] = NOT_GOAL_UP_PLAYER
    layout[grid == 'not_goal_right_player'] = NOT_GOAL_UP_PLAYER
    layout[grid == 'wall'] = WALL
    layout[grid == 'goal'] = PERSON
    assert np.max(layout) < 4
    return layout
...
def render(obs, mode='human', close=False):
    if mode == 'layout':
        return layout_from_string_grid(build_string_grid(obs))
    else:
