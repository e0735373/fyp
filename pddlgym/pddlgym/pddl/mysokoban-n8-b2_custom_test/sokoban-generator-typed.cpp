  ...
  while(!success){
   ...
   for(int i=0;i<num_rows;i++) {
      for(int j=0;j<num_cols;j++) {
        if (i == 0) {
          if (j == 0) {
            sokoban_world[i][j] = ROBOT;
          } else if (j+2 >= num_cols) {
            sokoban_world[i][j] = CLEAR;
          } else {
            sokoban_world[i][j] = WALL;
          }
        } else if (i == 1 || i == 4) {
          if (j == 1) {
            sokoban_world[i][j] = BOX;
            box_locs.push_back(make_pair(i,j));
          } else {
            sokoban_world[i][j] = CLEAR;
          }
        } else if (i == 2) {
...
void output_typed_strips(){
  ...
  // permute all goal_locs
  printf("\n(or");
  std::sort(goal_locs.begin(), goal_locs.end());
  do {
    printf("\n(and");
    print_goal_predicates();
    printf("\n)");
  } while (std::next_permutation(goal_locs.begin(), goal_locs.end()));
...
void print_all_other_predicates(){
  ...
  // is-goal predicate
  for (int i=0; i < goal_locs.size(); ++i) {
    printf("\n(is-goal f%d-%df) ", goal_locs[i].first, goal_locs[i].second);
  }
}
