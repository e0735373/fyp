...
def one_hot_with_ignore_index(x, num_classes, ignore_index=-100):
    x = x.long()
    assert x[x != ignore_index].min() >= 0 and x[x != ignore_index].max() < num_classes

    x_onehot = torch.nn.functional.one_hot(
        torch.where(x == ignore_index, torch.tensor(num_classes), x),
        num_classes + 1).float().to(x.device)
    x_onehot = x_onehot[..., :-1]
    x_onehot[x_onehot.sum(dim=-1) < 0.5] = 1.0 / num_classes
